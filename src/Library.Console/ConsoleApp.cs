...
public class ConsoleApp
{
    ...
    private async Task<ConsoleState> SearchBooks()
    {
        ...
        var matchingBook = _jsonData.Books?.FirstOrDefault(b => 
        ...
        // Get populated book with author information
        var populatedBook = _jsonData.GetPopulatedBook(matchingBook);

        // Find corresponding BookItem using BookId
        var bookItem = _jsonData.BookItems?.FirstOrDefault(bi => bi.BookId == matchingBook.Id);

        if (bookItem == null)
        {
            Console.WriteLine($"No physical copy of '{populatedBook.Title}' found in inventory.");
            return ConsoleState.PatronDetails;
        }

        // Check for active loan (ReturnDate == null) for this BookItem
        ...
        if (activeLoan == null)
        {
            ...
            Console.WriteLine($"{populatedBook.Title} is available for loan");
        }
        else
        {
            // Book is currently on loan
            Console.WriteLine($"{populatedBook.Title} is on loan to another patron. The return due date is {activeLoan.DueDate:yyyy-MM-dd}");
        }
